...
Use theory/viewpoint-led, literature/puzzle-led, policy/institution-led, data/measurement-led, and regional/spatial phenomenon-led research genesis. Generate multiple non-equivalent Core Questions, propositions, mechanisms, rivals, outcomes/constructs, and evidence architectures. Protect an independent scientific-divergence round before consulting data convenience. Convert viewpoints into falsifiable predictions; extract architecture from high-level papers without copying their setting or method. In the later part of Stage 1, use a light observability screen for candidates about to receive substantial further effort, then reopen or refine divergence rather than letting available variables anchor the candidate set. High-data-risk ideas remain eligible. No formal regressions.
...
For each candidate record research genesis, Core Question, scientific importance, Claim Type, mechanism, proposition/prediction, Evidence Architecture, setting/geography rationale, key construct, primary evidence, data, closest literature and paper architecture, residual contribution, main rival, main validity threat, workload, journal ceiling, and cheapest kill test. Apply `protocols/early_data_feasibility.md` before substantial further development: distinguish required, observable, and missing evidence; audit observability of both the core X/key construct and primary evidence; examine unit/identifier/time compatibility, reconstruction and manual-judgment burden, expected support, failure locality, and access states; then record qualitative Data Risk and the cheapest feasibility test. Compare scientific value and feasibility separately and avoid a shortlist concentrated in the same high-risk data architecture when stable output matters. Policy/shock, assignment, counterfactual, estimand, and identifying assumption are mandatory only for causal claims or causal-policy designs.
...
The Stage 1–2 early feasibility screen allocates research effort; it does not pass, replace, or lower any Stage 3 Gate. A promising pilot remains provisional until the formal Data / Measurement Gate is decided under its existing criteria.
